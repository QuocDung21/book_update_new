...
export const customer_block = createAsyncThunk(
  ...
  async (email, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.get(`/customer/block/${email}`);
      return fulfillWithValue(data);
    } catch (error) {
      return rejectWithValue(error.response.data);
    }
...
export const authReducer = createSlice({
  ...
  extraReducers: {
    [customer_block.pending]: (state, _) => {
      state.loader = true;
    },
    [customer_block.rejected]: (state, { payload }) => {
      state.errorMessage = payload.error;
      state.loader = false;
    },
    [customer_block.fulfilled]: (state, { payload }) => {
